export const REASONS_TO_DEPLOY = [
  "Eu te amo Sabiá",
  "Você é incrivel!",
  'Sim, eu sou o homem mais sortudo do mundo',
  'Eu ainda te amo',
  'Quer casar comigo?',
  'Eu não vou ser melhor sem você',
  'Por que você não esta me beijando agora? 😡',
  'Eu te amo em todos o 7 mares 🌊 \n Inclusive o mar de Minas',
  'Você é foda',
  'UaU, como você esta linda hoje',
  'Eu te amo Maíra',
  'We are more Ghosts than People',
  'Close your eyes bro',
  'Me liga 🥺',
  'Estou com saudades',
  'Finalmente estou usando programação pra alguma coisa',
  'Ja disse que te amo hoje?',
  'Você deve me achar o maior bobão',
  'Estou com saudades',
  'Quando eu digo que tem coisas mais importantes.. \n Você é a coisa mais importante',
  'Te mandei um presente no Pokemon GO <3',
  'Eu amo você',
  'Se for preciso eu viro esse mundo do avesso',
  'Case-se comigo agora. 😡',
  'Continue incrivel',
  'Dia ruim? Vai ficar tudo bem',
  'Estou pensando em você nesse exato momento',
  'Hoje eu sonhei com você',
  'que namorado mais boiola ein',
  'S+M 4ever',
  'Ainda estou com saudades',
  'Ainda estou pensando em você agora',
  'Maíra, eu te amo com todas as minhas forças',
  'Eu sempre vou estar aqui por você',
  '🎾?',
  'Ainda te amo',
  'Até que ta sendo divertido programar isso aqui',
  'Você é tudo pra mim',
  'I Could Die For You',
  'Take on me',
  'Essa é uma mensagem do Samuel do futuro: Eu te amo',
  'f(x)=x^{(2/3)}+0.85*(4-x^{2})^{(1/2)}/sin(a/pi*x)',
  'Eu vou estar aqui independente do que estiver acontecendo',
  'Its aways nice to see you oh oh oh',
  'Lebenslangerschicksalsschatz',
  'Ainda estou aqui, por você',
  'Esta livre para jantar hoje a noite? Então é um encontro',
  '01100101 01110101 00100000 01110100 01100101 00100000 01100001 01101101 01101111',
  '. ..- / - . / .- -- ---',
  'Você acaba de ganhar um beijo! Printe esta tela, envie para o criador do site e retire já o seu beijo! obs: Valido somente para Maíra'
]

export const REASONS_TO_NOT_DEPLOY = [
  //...REASONS_TO_DEPLOY,
  "É hoje! Estou te esperando.",
  "Bem que você podia deixar eu te buscar né",
  'Estou ansioso pra te ver',
  'Me avisa quando estiver indo',
  "Se estiver calor eu vou te levar sorvete 🍦",
  'Oh Oh Oh Oh \n I got a love that keeps me waiting',
  'Eu te amooooooooo'
]

export const REASONS_FOR_FRIDAY_AFTERNOON = [
  'Sextaaaaaaaaaaaaaaa',
  'Já é sexta, sera que você ja chegou?',
  'Ei, Ei, já chegou xuxu?',
  'Eu acho que você bem que podia me ver hoje né',
  'Eu quero ser o primeiro a te ver',
  'Chegou bem meu docinho?',
  'Eu te amo',
  'Dont take me tongue tied'
]

export const REASONS_FOR_28TH = [
 'Feliz aniversario de namoro amor',
 'Mais um mes!',
 'A gente ja casou?',
 'Eu te amo, obrigado por fazer parte do meu mundo',
 'Eu tenho você, então nada mais importa.'
]

export const REASONS_FOR_AFTERNOON = [
  ...REASONS_TO_DEPLOY,
  'Já esta ficando tarde, eu te amo',
  'Que tal voltar amanhã?',
  'Eu te amo mais do que o sol ama a lua',
  'Você devia casar comigo',
  'Soninho?',
  'Dream a little dream of me'
]

export const REASONS_FOR_WEEKEND = [
  ...REASONS_TO_DEPLOY,
  'é fim de semana, era pra você estar comigo agora',
  'a gente vai se ver hoje?',
  'por que você ta aqui e não comigo? 😡',
  'eu vou te encher de beijinhos',
  'vem me veeeeeer',
  'estou contando os segundos pra te ver',
  'Triste saber que você tem que ir embora'
]

export const REASONS_FOR_DAY_BEFORE_CHRISTMAS = [
  'é natal 🎄 é natal',
  '🎶🎵 Last Christmas I gave you my heart 🎵🎶',
  '🎄 Ja te entreguei seu presente? 🎄 ',
  'Dingo Bell Dingo Bell',
  'Vou passar o natal com você',
  'Vamos assistir Klaus?'
]

export const REASONS_FOR_CHRISTMAS = [
  ...REASONS_FOR_DAY_BEFORE_CHRISTMAS,
  'Agora sim, oficialmente natal ',
  'Cade meu presente?',
  'Eu te amo em qualquer dia do ano'
]

export const REASONS_NEW_YEAR = [
  'Feliz Ano Novo! \n Eu te amo meu amor',
  'Que cor de roupa eu uso pra ter você o ano todo?',
  'Eu te amo Maíraaaaa',
  'Fogoooossss 🎇'
]
